...
// The MSG91 template names actually in use are NOT the plain niche id —
// they're built from the AIDA dashboard's own NICHE_SHORT label (see
// skyup-aida-dashboard-final.html, buildMain(): `${shortName}_${stage}_v${n}`
// .toLowerCase()`), which for several niches is a longer, differently-worded
// string. E.g. the niche "ai" actually maps to the MSG91 prefix
// "ai_automation" (from label "AI_Automation"), not "ai" — so the real,
// already-approved template is "ai_automation_awareness_v1", not
// "ai_awareness_v1". Using the plain niche id caused every non-trivial niche
// (everything except general/crm/website, which happen to match their own
// id) to look "missing" even though the templates already existed and were
// Enabled in MSG91 the whole time.
...
/**
 * Build a niche fallback template name — using the ACTUAL MSG91 name prefix
 * for this niche (see NICHE_TEMPLATE_PREFIX above), not the plain niche id.
